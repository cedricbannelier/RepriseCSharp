...
namespace MonSiteMvc.Controllers
{
    ...
    public class BddController : ControllerBase
    {
        ...
        {
            if (dto == null)
                return BadRequest(new { succes = false, message = "Données manquantes" });

            if (string.IsNullOrWhiteSpace(dto.Userlogin))
                return BadRequest(new { succes = false, message = "Le champ Nom est vide." });

            if (dto.Userpwd == null)
                return BadRequest(new { succes = false, message = "Vous devez renseigner un mot de passe" });
